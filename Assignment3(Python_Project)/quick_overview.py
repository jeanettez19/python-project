...
class QuickOverview:
    def __init__(self):
        try:
            ...
            #self.category = pd.read_excel("./data/categories.xlsx")
            ...
            self.budget = pd.DataFrame(columns=["budget_id", "date", "category", "monthly_budget"])
            #self.category = pd.DataFrame(columns=["category_id", "category_name"]
    ...
    def get_current_month_overview(self):
        ...
        # Filter income data for the current month and year
