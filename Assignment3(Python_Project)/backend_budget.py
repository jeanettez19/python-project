...
class Budget:
    ...
    def get_all_budgets(self):
        """Retrieve all budget in a orient format sorted by the most recent date."""
        try:
            # Sort the DataFrame by the 'date' column in descending order
            sorted_budgets = self.__budget_df.sort_values(by='date', ascending=False)
            # Convert the DataFrame to a dictionary in 'records' format
            budgets = sorted_budgets.to_dict(orient='records')
            return budgets
        except Exception as e:
